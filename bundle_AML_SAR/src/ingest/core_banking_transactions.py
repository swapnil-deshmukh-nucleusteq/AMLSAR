...
# INGEST: raw_sources.core_banking_transactions  |  5 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "core_banking_transactions")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.core_banking_transactions")
...
df = (df
    ...
    .withColumn("txn_amount", F.col("txn_amount").cast("decimal(18,4)"))
...
dbutils.jobs.taskValues.set(key="core_banking_transactions_count", value=count)
