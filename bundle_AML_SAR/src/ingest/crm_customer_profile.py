...
# INGEST: raw_sources.crm_customer_profile  |  3 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "crm_customer_profile")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.crm_customer_profile")
...
df = (df
    ...
    .withColumn("kyc_review_date", F.col("kyc_review_date").cast("date"))
    .withColumn("onboarding_date", F.col("onboarding_date").cast("date"))
    .withColumn("src_modified_ts", F.col("src_modified_ts").cast("timestamp"))
)
