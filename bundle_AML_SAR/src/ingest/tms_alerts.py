...
# INGEST: raw_sources.tms_alerts  |  3 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "tms_alerts")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.tms_alerts")
...
# Type casts
df = (df
    .withColumn("alert_score", F.col("alert_score").cast("double"))
    .withColumn("alert_date", F.col("alert_date").cast("timestamp"))
    .withColumn("linked_txn_ids", F.split(F.col("linked_txn_ids").cast("string"), "\\|"))
)
...
dbutils.jobs.taskValues.set(key="tms_alerts_count", value=count)
