...
# INGEST: raw_sources.watchlist_screening  |  1 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "watchlist_screening")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.watchlist_screening")
...
# Type casts
df = (df
    .withColumn("match_score", F.col("match_score").cast("double"))
    .withColumn("screened_ts", F.col("screened_ts").cast("timestamp"))
)
...
dbutils.jobs.taskValues.set(key="watchlist_screening_count", value=count)
