...
# Upstream tables were already created by bootstrap and populated by prior tasks
    src = spark.table('`{CATALOG}`.`raw_sources`.`core_banking_transactions`')
    df = (src
    ...
    _cols = [c for c in ['txn_sk', 'txn_id', 'customer_sk', 'txn_date', 'txn_amount_usd', 'txn_type', 'is_cash', 'ctr_eligible', 'dq_passed', 'dq_flags'] if c in src.columns]
