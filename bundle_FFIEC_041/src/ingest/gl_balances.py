...
# INGEST: raw_sources.gl_balances  |  5 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "gl_balances")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.gl_balances")
...
df = (df
    ...
    .withColumn("ytd_amount_usd", F.col("ytd_amount_usd").cast("decimal(18,4)"))
...
dbutils.jobs.taskValues.set(key="gl_balances_count", value=count)
