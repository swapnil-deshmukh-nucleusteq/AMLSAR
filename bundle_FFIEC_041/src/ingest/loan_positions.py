...
# INGEST: raw_sources.loan_positions  |  7 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "loan_positions")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.loan_positions")
...
# Type casts
df = (df
    .withColumn("outstanding_balance", F.col("outstanding_balance").cast("decimal(18,4)"))
    .withColumn("unearned_income", F.col("unearned_income").cast("decimal(18,4)"))
    .withColumn("origination_amount", F.col("origination_amount").cast("decimal(18,4)"))
    .withColumn("origination_date", F.col("origination_date").cast("date"))
    .withColumn("maturity_date", F.col("maturity_date").cast("date"))
    .withColumn("next_reprice_date", F.col("next_reprice_date").cast("date"))
    .withColumn("days_past_due", F.col("days_past_due").cast("int"))
    .withColumn("charge_off_amount", F.col("charge_off_amount").cast("decimal(18,4)"))
    .withColumn("recovery_amount", F.col("recovery_amount").cast("decimal(18,4)"))
    .withColumn("held_for_sale_flag", F.col("held_for_sale_flag").cast("boolean"))
    .withColumn("is_small_business", F.col("is_small_business").cast("boolean"))
...
dbutils.jobs.taskValues.set(key="loan_positions_count", value=count)
