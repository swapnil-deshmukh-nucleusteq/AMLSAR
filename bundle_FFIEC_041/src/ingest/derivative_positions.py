...
# INGEST: raw_sources.derivative_positions  |  3 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "derivative_positions")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.derivative_positions")
...
# Type casts
df = (df
    .withColumn("notional_amount", F.col("notional_amount").cast("decimal(18,4)"))
    .withColumn("fair_value_positive", F.col("fair_value_positive").cast("decimal(18,4)"))
    .withColumn("fair_value_negative", F.col("fair_value_negative").cast("decimal(18,4)"))
    .withColumn("cleared_flag", F.col("cleared_flag").cast("boolean"))
...
dbutils.jobs.taskValues.set(key="derivative_positions_count", value=count)
