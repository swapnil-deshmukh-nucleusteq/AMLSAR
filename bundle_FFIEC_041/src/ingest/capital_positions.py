...
# INGEST: raw_sources.capital_positions  |  2 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "capital_positions")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.capital_positions")
...
# Type casts
df = (df
    .withColumn("amount_usd", F.col("amount_usd").cast("decimal(18,4)"))
    .withColumn("risk_weight_pct", F.col("risk_weight_pct").cast("decimal(6,4)"))
    .withColumn("rwa_amount", F.col("rwa_amount").cast("decimal(18,4)"))
    .withColumn("cblr_election", F.col("cblr_election").cast("boolean"))
    .withColumn("cecl_transition_flag", F.col("cecl_transition_flag").cast("boolean"))
...
dbutils.jobs.taskValues.set(key="capital_positions_count", value=count)
