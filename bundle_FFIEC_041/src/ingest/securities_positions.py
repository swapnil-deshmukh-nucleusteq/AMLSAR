...
# INGEST: raw_sources.securities_positions  |  4 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "securities_positions")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.securities_positions")
...
# Type casts
df = (df
    .withColumn("gse_flag", F.col("gse_flag").cast("boolean"))
    .withColumn("amortized_cost", F.col("amortized_cost").cast("decimal(18,4)"))
    .withColumn("fair_value", F.col("fair_value").cast("decimal(18,4)"))
    .withColumn("unrealized_gl", F.col("unrealized_gl").cast("decimal(18,4)"))
    .withColumn("maturity_date", F.col("maturity_date").cast("date"))
    .withColumn("next_reprice_date", F.col("next_reprice_date").cast("date"))
    .withColumn("acl_amount", F.col("acl_amount").cast("decimal(18,4)"))
    .withColumn("pledged_flag", F.col("pledged_flag").cast("boolean"))
...
dbutils.jobs.taskValues.set(key="securities_positions_count", value=count)
