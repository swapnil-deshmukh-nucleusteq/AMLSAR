...
# INGEST: raw_sources.deposit_accounts  |  4 DQ rules  |  source_mapping driven

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "deposit_accounts")
...
if CONN_TYPE == "volume":
    ...
    df = (spark.read.format("jdbc")
        ...
        .option("user",     dbutils.secrets.get(SECRET_SCOPE, "jdbc_user"))
        .option("password", dbutils.secrets.get(SECRET_SCOPE, "jdbc_password"))
        .option("dbtable",  "raw_sources.deposit_accounts")
...
# Type casts
df = (df
    .withColumn("interest_bearing_flag", F.col("interest_bearing_flag").cast("boolean"))
    .withColumn("balance_usd", F.col("balance_usd").cast("decimal(18,4)"))
    .withColumn("accrued_interest", F.col("accrued_interest").cast("decimal(18,4)"))
    .withColumn("maturity_date", F.col("maturity_date").cast("date"))
    .withColumn("next_reprice_date", F.col("next_reprice_date").cast("date"))
    .withColumn("brokered_flag", F.col("brokered_flag").cast("boolean"))
    .withColumn("reciprocal_flag", F.col("reciprocal_flag").cast("boolean"))
    .withColumn("retirement_flag", F.col("retirement_flag").cast("boolean"))
    .withColumn("insured_flag", F.col("insured_flag").cast("boolean"))
...
dbutils.jobs.taskValues.set(key="deposit_accounts_count", value=count)
