...
# TRANSFORM: ffiec041_schedules.schedule_rc_c  |  upstream: fact_loans_conformed  |  aggregate_schedule

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "schedule_rc_c")
...
TARGET      = f"`{CATALOG}`.`ffiec041_schedules`.`schedule_rc_c`"
...
# Upstream tables were already created by bootstrap and populated by prior tasks
    ...
    _cols = [c for c in ['rcon_code', 'line_item', 'description', 'column_ref', 'amount_usd_000', 'report_date', 'validated'] if c in src.columns]
...
dbutils.jobs.taskValues.set(key="schedule_rc_c_count", value=count)
