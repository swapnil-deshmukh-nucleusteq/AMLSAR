...
# TRANSFORM: ffiec041_schedules.schedule_ri  |  upstream: fact_gl_positions  |  aggregate_schedule

# COMMAND ----------
dbutils.widgets.text("catalog",     "bfsi_regulatory_catalog")
dbutils.widgets.text("report_date", "2026-03-31")
dbutils.widgets.text("table_name",  "schedule_ri")
...
TARGET      = f"`{CATALOG}`.`ffiec041_schedules`.`schedule_ri`"
...
# Upstream tables were already created by bootstrap and populated by prior tasks
    ...
    _cols = [c for c in ['riad_code', 'line_item', 'description', 'ytd_amount_usd_000', 'report_date', 'validated'] if c in df.columns]
...
dbutils.jobs.taskValues.set(key="schedule_ri_count", value=count)
